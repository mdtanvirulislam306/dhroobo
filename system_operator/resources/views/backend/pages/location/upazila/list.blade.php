...
@section('title', 'Upazila List - ' . config('concave.cnf_appname'))
@section('content')
    <div class="grid-margin stretch-card">
        <div class="card">
            <div class="card-body">
                <span class="card-title">Dashboard > Location > Upazila</span>
                <a class="btn btn-success float-right" href="{{ route('admin.location.upazila.create') }}">Create New
                    District</a>
            </div>
        </div>
    </div>
                ...
                <table id="dataTable" class="table">
                    ...
                    <tbody>
                        {{-- @foreach ($data as $d)
                            <tr>
                                <td>{{ $d->id }}</td>
                                <td>{{ $d->district->title ?? '' }}</td>
                                <td>{{ $d->title }}</td>
                                <td class="text-center">
                                    @if (Auth::user()->can('location.edit'))
                                        <a class="text-success" href="{{ route('admin.location.upazila.edit', $d->id) }}"><i
                                                class="mdi mdi-pencil-box-outline"></i></a>
                                    @endif
                                    @if (Auth::user()->can('location.delete'))
                                        <a class="text-danger delete_btn"
                                            data-url="{{ route('admin.location.upazila.delete', $d->id) }}"
    ...
    <script type="text/javascript">
        function fetchData() {
            ...
            var table = jQuery('#dataTable').DataTable({
                ...
                ajax: {
                    url: "{{ url('admin/location/upazila/get-upazila') }}",
                    type: 'GET',
                },
                aLengthMenu: [
