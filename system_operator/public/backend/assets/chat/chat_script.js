...
class ChatApp{
    ...
    getChat(){
        ...
        this.ChatRooms.doc(docID).collection("messages").orderBy("timestamp").get().then((snapshot) => {
            ...
            snapshot.docChanges().forEach(function(value,index){
                ...
                if( data.toUserId == localStorage.getItem('userID')){
                    htmlMessage +=  '<div class="customer_message"><p>'+data.text+'</p></div>';
                }else{
                    if(jQuery('.selected_chat').attr('data-user-id') != data.senderId){
                        jQuery('div[data-user-id="'+data.toUserId+'"]').addClass('new_message');
                    }else{
            ...
            jQuery('.chating_title_dynamic').html(localStorage.getItem('userName'));
    ...
    initChatList(){
        localStorage.setItem('chatInitiated',1);
        let docID = localStorage.getItem('sellerID')+localStorage.getItem('userID');
        //let audio = new Audio('/backend/assets/chat/sound.mp3');
        this.ChatRooms.doc(docID).collection("messages").orderBy("timestamp").onSnapshot(function(snapshot){
            ...
            snapshot.docChanges().forEach(function(value,index){
                ...
                if(data.senderId == localStorage.getItem('sellerID')){
                    htmlMessage +=  '<div class="seller_message"><p>'+data.text+'</p></div>';
                }else{
                    if(jQuery('.selected_chat').attr('data-user-id') != data.senderId){
                        jQuery('div[data-user-id="'+data.senderId+'"]').addClass('new_message');
                       // audio.play();
                    }else{
                        htmlMessage +=  '<div class="customer_message"><p>'+data.text+'</p></div>';
                      //  audio.play();
            ...
            allData.forEach(function(val,key){
                ...
                if(val.usersid[0] != null){
                    if(val.usersid[0].toString() == localStorage.getItem('sellerID').toString()){
                        ...
                        let isSeen = '';
                        if(val.seenbyseller != 1){
                            isSeen = 'chat_unread'
                        }

                        formatedHtml+='<div data-user-id="'+val.usersid[1]+'" data-user-name="'+val.fromusername+'" class="row user_section '+isSeen+'">'+
                                '<div class="col-md-2 col-lg-2 p-0">'+
                                '<img style="width:80%" src="'+avater+'" alt="avater">'+
                                '</div>'+
                                '<div class="col-md-10 col-lg-10">'+
                                '<span class="seller_item_title">'+val.fromusername+'</span> <p>'+LocalDate+' '+LocalTime+'</p>'+
                                '</div>'+
                            '</div>';
                    }
                }
    ...
    sendMessage(object){
        ...
        if(localStorage.getItem('userID') == null){
            alert('Please select a User to generate chat!');
            return;
        }
    ...
    seenMesage(){
        let docID = localStorage.getItem('sellerID')+localStorage.getItem('userID');
        this.ChatRoomUsers.doc(docID).update({seenbyseller: 1});
    }

    chatCount(){
        let data = this.ChatRooms.collection("messages").where("text", "==", "hello");
        //console.log(data);
...
jQuery(document).on('click','#btn-chat',function(){
    ...
    if(message){
        chatClass.sendMessage({
            senderId : localStorage.getItem('sellerID'),
...
jQuery(document).on('click','.user_section',function(){
    jQuery('.user_section').removeClass('selected_chat');
    jQuery(this).addClass('selected_chat');
    jQuery(this).removeClass('new_message');
    jQuery(this).removeClass('chat_unread');
    jQuery('.message_input').show();
    localStorage.setItem('userID',jQuery(this).attr('data-user-id'));
    localStorage.setItem('userName',jQuery(this).attr('data-user-name'));
    jQuery('.comment_section').html('');

    chatClass.seenMesage()
    chatClass.getChat();
...
jQuery(document).ready(function(){
    chatClass.initChat();
    localStorage.setItem('chatInitiated',0);
});

jQuery(document).on('click', '.chating-box', function() {
    jQuery('.chating_wrapper').fadeIn();

    if(localStorage.getItem('chatInitiated') != 1){
...
jQuery(document).on('click', '.close_chating', function() {
    jQuery('.chating_wrapper').fadeOut();
});
